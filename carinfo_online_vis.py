...
def observerRun(path):
    ...
    try:
        while True:
            ...
            plt.subplot(4, 1, 2)
            plt.axhline(0, linestyle='--', c='gray')
            plt.plot(data_pd['frame'].astype('Int64'), data_pd['Ori-velocity'] - data_pd['pose-velocity'],color='b', label='diff_velocity')
            plt.plot(data_pd['frame'].astype('Int64'), data_pd['Ori-yawrate'] - data_pd['pose-yawrate'],color='r', label='diff_yawrate')
            plt.xticks(data_pd['frame'].astype('Int64'), rotation=270)
            plt.legend(loc='upper left')
            plt.subplot(4, 1, 3)
            plt.axhline(0, linestyle='--', c='gray')
